...
- `书库`：读取桌面端 SQLite 索引，支持 `全部`、JM 号、作者和标签搜索；`全部` 会显示当前结果总数，封面卡片按作者分组，并根据窗口宽度自动调整每行数量；批量管理模式支持多选、全选、反选、取消全选和删除所选本地漫画。
- `下载`：输入一个或多个 JM 号，任务会立即进入列表；下载时显示总进度、当前任务状态，并为失败任务保留重试入口。
