...
- 桌面端用 PySide6 + QFluentWidgets 封装书库、下载、查看详情、书库修复、禁漫官网和设置页。
...
- 标题和作者保持来源文本；标签统一转换为中文简体后写入 `catalog.md` 和桌面端 SQLite 索引，方便搜索和分类筛选。

### PDF

下载完成后通过 `img2pdf` 插件生成 PDF。桌面端下载服务会在 PDF 生成后整理目录：
...
- 按作者分组。
- 每个作者下独立编号。
- 同一作品如有多个作者，只登记到第一作者下，避免目录和书库重复计数。
- 同一作者下按 JM ID 去重更新，不重复追加。
- 封面优先使用本次下载成功的第一章第一张图片。
- 封面以内嵌 base64 写入 Markdown。
- 默认封面宽度为 `120`。
- 只对标签做繁简统一，不改标题、作者、章节标题和封面内容。
- 桌面端重建索引和批量删除后，会按下载目录中实际存在的本地漫画同步清理 `catalog.md` 旧条目，避免目录、SQLite 和书库数量长期不一致。
...
- 左侧“禁漫官网”页位于“书库修复”和底部“设置”之间，通过 Fluent 分组卡片展示发布页、分流域名、APP 下载和联系方式。
- 点击地址时使用 `QUrl.fromUserInput()` 解析用户提供的原始文本，再由 `QDesktopServices.openUrl()` 交给系统默认浏览器。
- 裸域名保持用户提供的原样，不在页面数据中强制补写 `https://`。
- 页面不内嵌 WebEngine，不在启动或打开页面时主动探测域名，避免新增依赖和地区网络误判。
...
根目录 `TASKS.md` 用于记录当前未完成的目标和步骤，方便上下文压缩或新开对话后继续工作。

- 开始任何需要修改代码、脚本或文档的任务前，先读取 `TASKS.md`。
- 如果本次任务尚未记录，先把目标拆成可勾选步骤写入 `TASKS.md`。
- 每完成一个步骤，及时把对应条目标记为已完成。
- 全部任务完成、验证通过并准备交付后，清空本次任务，只保留“暂无进行中任务”。
- `TASKS.md` 只记录任务和状态，不记录账号、密码、cookie、token、本地下载内容、PDF、封面或真实私有配置。
